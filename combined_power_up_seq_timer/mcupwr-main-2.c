...
int soc_init(coulomb_count,h){
	...
	while(sample_counter < sample_size){
		sampled_i[sample_counter] = solar1_current;
		sampled_v[sample_counter] = batt1_voltage;
		sample_counter++;
		delay(10);
	}
...
void soc_data_gather(double sampled_i[],double sampled_v[],double h[]){
	...
	int ixh = 0;
	while(sample_counter < sample_size){
		double voltage = batt1_voltage;
		scale_adc_volt(batt1_voltage);
		sampled_i[sample_counter] = solar1_current;
		sampled_v[sample_counter] = voltage;
...
void scale_adc_volt(double adc_volty){
	
	// Some Math
	adc_volty = termVolt[50];
...
void StateofCharge(){
	...
	*/
	...
	// sampled_i = solar1_current; //convert this value;
	// sampled_v = batt1_voltage;  //need a function to convert batt1_voltage to a double
	
	// scale_adc_volt(sampled_v);
	...
	// soc_look_up(OCV,SOCv);
	compute_h(OCV,vf,uf,h);
	SOCv = OCV; //Global is set
	
	// Ignoring what is below this... old code
